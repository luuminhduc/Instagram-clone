...
const EditPost = () => {
	...
	const handlePost = () => {
		if (watch("text") && watchImg && watchImg.length > 0) {
			const post = {
				text: watch("text"),
				files: watch("images"),
				time: timeStamp(),
			...
			dispatch(showModal({ text: "Please add some images and title" }));
		}
	};
	...
	return currentUser ? (
		<div className="w-full bg-white p-5 rounded-sm border border-solid border-gray-200">
			...
			<div className="border-t border-solid border-gray-200 pt-5 flex flex-row justify-between items-start">
				...
				<button
					onClick={handlePost}
					className={`px-7 py-3  text-white cursor-pointer bg-blue-600 hover:bg-blue-500  rounded  focus:outline-none`}
				>
					Post
				</button>
			</div>
